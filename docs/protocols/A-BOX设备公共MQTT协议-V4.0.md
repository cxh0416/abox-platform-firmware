...
生产连接最低使用 TLS 1.2，支持时优先 TLS 1.3。设备必须校验服务端证书链、主机名和证书有效时间，并发送正确 SNI。每台设备必须使用独立 Broker 凭据；凭据轮换属于受控配置或生产 provisioning，不通过公共业务 Topic 传输，切换完成后必须撤销旧凭据。隔离实验 Broker 只有在显式非生产配置下才允许明文连接，生产端点不得例外。
